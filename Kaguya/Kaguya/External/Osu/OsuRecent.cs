...
namespace Kaguya.External.Osu
{
	public class OsuRecent : OsuBase
	{
		...
		public OsuRecent(OsuData osuData) : base(osuData.Client) { _osuData = osuData; }

		public async Task<Embed> GetMostRecentForUserAsync()
		{
			var osuUser = await _osuData.GetOsuUserAsync();
			var userRecentPlays = await _osuData.Client.GetUserRecentsByUserIdAsync(osuUser.UserId, _osuData.GameMode);
			...
			var recentMapInfo = await recentPlay.GetBeatmapAsync();
			var recentPpInfo = await recentPlay.GetPPAsync();
			...
			string starInformation = "Stars:".AsBold() +
			                         $" {(recentPpInfo.Stars == 0 ? recentMapInfo.StarRating.Value.ToString("N2") : recentPpInfo.Stars.ToString("N2"))}★";
...
#region Hit Count Information
			...
			if (has100 || has50 || hasMisses)
			{
				...
				foreach (string element in missDisplay)
				{
					final += element + ", ";
				}
