...
namespace Kaguya.Discord.Commands.Configuration
{
	...
	public class Antiraid : KaguyaBase<Antiraid>
	{
		...
		public async Task AntiraidSetupCommand()
		{
			...
			while (!stageOneClear)
			{
				var userMessage = await _interactivityService.NextMessageAsync(x => x.Author == Context.User, null,
			...
			if (newArConfig.Action == ModerationAction.Mute || newArConfig.Action == ModerationAction.Ban)
			{
				...
				if (confirmation.IsSuccess)
				{
					...
					while (true)
					{
						var nextMessage = await _interactivityService.NextMessageAsync(x => x.Author == Context.User,
			...
			string timeString = null)
		{
			...
			var parsedAction = GetAntiraidActionEnum(action);
			...
			if (parsedAction != ModerationAction.Kick && timeString != null)
			{
				if (!ValidatePunishmentDurationInput(timeString))
				{
					...
					await SendBasicErrorEmbedAsync(
						$"I couldn't parse your punishment duration. Please review the `{server.CommandPrefix}help antiraid` command " +
		...
		[Summary("Sets and enables the antiraid direct messages (DMs) for this server. Antiraid DMs " +
		         ...
		         "__Notice:__\n" +
		         "Servers with Antiraid DMs that violate Discord's " +
		         Global.DiscordTermsLink +
		         " or " +
		         Global.DiscordCommunityGuidelinesLink +
		         " " +
		...
		private async Task<bool> ConfirmConfigOverwriteAsync(AntiRaidConfig config)
		{
			...
			var pageBuilder = new PageBuilder().WithDescription(
				                                   $"{Context.User.Mention} I see that your server already has a defined anti-raid configuration:\n\n" +
				                                   $"{GetAntiraidConfigString(config)}\n" +
				                                   "Are you sure you would like to change this config?")
			                                   .WithColor(KaguyaColors.IceBlue);

			var builder = new ConfirmationBuilder().WithContent(pageBuilder)
			                                       .WithUsers(Context.User)
		...
		private string GetAntiraidConfigString(AntiRaidConfig config)
		{
			...
			configStringBuilder.AppendLine(
				$"Time threshold: {TimeSpan.FromSeconds(config.Seconds).Humanize(3, minUnit: TimeUnit.Second).AsBold()}");

			string userPunishDuration = "User punishment duration: ";

			if (config.Action == ModerationAction.Ban ||
			    config.Action == ModerationAction.Mute ||
			    config.Action == ModerationAction.Shadowban)
			{
		...
		{
			var embed = new KaguyaEmbedBuilder(KaguyaColors.LightYellow).WithDescription(
				$"The action `{wrongInput}` is not a valid action. Please try again with one of the following:\n" +
		...
		{
			return new KaguyaEmbedBuilder(KaguyaColors.NeonPurple).WithTitle("Anti Raid: User Threshold")
			                                                      .WithDescription(
				                                                      $"{Context.User.Mention} Okay, let's setup the anti raid for your server.\n\n" +
		...
		{
			return new KaguyaEmbedBuilder(KaguyaColors.NeonPurple).WithTitle("Anti Raid: Duration Threshold")
			                                                      .WithDescription(
				                                                      $"{Context.User.Mention} Great. Now, what is the duration (in seconds) that {userThreshold:N0} users joining " +
		...
		private Embed GetStageThreeEmbed(uint userThreshold, uint seconds)
		{
			return new KaguyaEmbedBuilder(KaguyaColors.NeonPurple).WithTitle("Anti Raid: Action")
			                                                      .WithDescription(
				                                                      ...
				                                                      "Shadowbanned users cannot interact with the server in any way (cannot see channels, chats, other members, etc.)"
					                                                      .AsItalics())
			                                                      .Build();
		}

		// todo: Figure out why action is not used.
		private Embed GetStageFourConfirmationEmbed(ModerationAction action)
		{
			return new KaguyaEmbedBuilder(KaguyaColors.Magenta).WithTitle("Anti Raid: Temporary Action Setup")
			                                                   .WithDescription(
				                                                   $"{Context.User.Mention} It looks like you've set an action that can also be lifted after a duration.\n" +
		...
		{
			return new KaguyaEmbedBuilder(KaguyaColors.NeonPurple).WithTitle("Anti Raid: Punishment Duration")
			                                                      .WithDescription(
				                                                      $"{Context.User.Mention} How long should the user be punished?\n\n" +
		...
		{
			return new KaguyaEmbedBuilder(KaguyaColors.Green).WithTitle("Anti Raid: Setup Complete")
			                                                 .WithDescription(
				                                                 $"{Context.User.Mention} Awesome! I've configured the antiraid for your server. Here's what I've got:\n\n" +
				                                                 $"{GetAntiraidConfigString(config)}")
			                                                 .WithFooter(
				                                                 $"Turn this off at any time through the \"{serverCmdPrefix}antiraid -toggle\" command.\n" +
		...
		/// </summary>
		/// <param name="id"></param>
		private static void Remove(ulong id) { _currentlyActiveSetups.TryRemove(id, out bool _); }
