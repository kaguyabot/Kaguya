...
namespace Kaguya.Discord.Commands.Configuration
{
    ...
    public class LevelPreferences : KaguyaBase<LevelPreferences>
    {
        ...
        public async Task SetNotificationsCommandAsync()
        {
            ...
            }.WithFooter($"Respond with only a number. Example: 1 | Current config: {server.LevelNotifications.Humanize(LetterCasing.Title)}");
            ...
            while (true)
            {
                ...
                msg = await _interactivityService.NextMessageAsync(x => int.TryParse(x.Content, out res) ||
            ...
            {
                modifyString = $"Notifications are currently being sent to {Context.Guild.GetTextChannel(server.LevelAnnouncementsChannelId!.Value).Mention}.\n" +
            ...
            {
                await SendBasicEmbedAsync("Where do you want to send the level notifications?\nExample: `#my-channel`", KaguyaColors.Purple);
                var response = await _interactivityService.NextMessageAsync(x => x.MentionedChannels.Any(), timeout: TimeSpan.FromSeconds(60));
