...
namespace Kaguya.Discord.Commands.Administration
{
	...
	public class Mute : KaguyaBase<Mute>
	{
		...
		public async Task UnmuteUserCommand(SocketGuildUser user)
		{
			...
			var allUserMutes = await GetUnexpiredMutesAsync(user.Id, server.ServerId);
		...
		public async Task MuteSyncCommand()
		{
			...
			var embed = GetBasicSuccessEmbedBuilder($"Synced permissions for the mute role {muteRole.Mention}.")
		...
		private async Task MuteUserAsync(SocketGuildUser user, DateTimeOffset? expiration, string reason, KaguyaServer server)
		{
			var adminAction = new AdminAction
			{
				...
				HasTriggered =
					expiration.HasValue
						? false
						: null // We specify this value if the user is temporarily actioned. Otherwise, leave it null.
			};
			...
			catch (Exception e)
			{
				await SendBasicErrorEmbedAsync(
					...
					"The mute operation will still continue. " +
					"Use the ".AsBold() +
					"mute -sync".AsCodeBlockSingleLine().AsBold() +
					" " +
			...
			{
				await SendBasicErrorEmbedAsync(
					$"Failed to add role {muteRole.ToString().AsBold()} to user {user.ToString().AsBold()}.\nReason: {e.ToString().AsBold()}");
		...
		{
			var currentUserMutes = await GetUnexpiredMutesAsync(user.Id, Context.Guild.Id);
			...
			var longestMute = permanentMute ? currentUserMutes.First(x => !x.Expiration.HasValue) : currentUserMutes[0];
			...
			string oldMuteDurationStr = (permanentMute ? "never".AsBold() : longestMute.Expiration.Humanize())
			...
			var oldMod = Context.Guild.GetUser(longestMute.ModeratorId);
			...
			else
			{
				await SendBasicSuccessEmbedAsync(
					"Okay, I'll insert this and log it, but if the user currently has mutes that expire later " +
		...
		private bool DetermineIfMuteRoleExists(KaguyaServer server)
		{
			...
			var muteRole = Context.Guild.GetRole(server.MuteRoleId.Value);
			return muteRole != null;
		}

		private async Task<IRole> GetMuteRoleAsync(KaguyaServer server)
		{
			var match = Context.Guild.GetRole(server.MuteRoleId ?? 0) ?? await CreateMuteRoleAsync();
			return match;
		}
		...
		private async Task<bool> UserIsCurrentlyMutedAsync(SocketGuildUser user, IRole muteRole)
		{
			...
			var unexpiredUserMutes = await GetUnexpiredMutesAsync(user.Id, Context.Guild.Id);
