...
namespace Kaguya.Discord.Commands.Administration
{
	...
	public class Filter : KaguyaBase<Filter>
	{
		...
		{
			var filter = await _fwRepo.GetAllAsync(Context.Guild.Id, true);
			...
			{
				var page = new PageBuilder().WithTitle("Word Filter").WithColor(KaguyaColors.Magenta);
		...
		[Priority(0)]
		[Command("-a", RunMode = RunMode.Async)]
		public async Task CommandAddToFilter([Remainder]
			string word)
		{
			var fw = new FilteredWord
			{
		...
		[Summary("Removes a word or phrase from the word filter. If the word is a wildcard, you " +
		         ...
		         "")]
		[Remarks("<word>")]
		public async Task CommandRemoveFromFilter([Remainder]
			string word)
		{
		...
		{
			var curFilters = await _fwRepo.GetAllAsync(Context.Guild.Id, true);
			if (!curFilters.Any())
			{
