...
namespace Kaguya.Discord.Commands.Reference
{
	...
	public class Redeem : KaguyaBase<Redeem>
	{
		...
		[Command]
		[Summary("Used to redeem a [Kaguya Premium](" +
		         Global.StoreUrl +
		         ") key.\n\n" +
		...
		public async Task RedeemCommand(params string[] keys)
		{
			...
			{
				var match = await _premiumKeyRepository.GetKeyAsync(key);
				if (match == null)
				{
				...
				var userPremiumRemaining = user.PremiumExpiration.Value - DateTimeOffset.Now;
				var serverPremiumRemaining = server.PremiumExpiration.Value - DateTimeOffset.Now;
