...
namespace Kaguya.Discord.Commands.Music
{
    ...
    {
        private readonly ILogger<Loop> _logger;
        private readonly LavaNode _lavaNode;
        private readonly InteractivityService _interactivityService;

        public Loop(ILogger<Loop> logger, LavaNode lavaNode, InteractivityService interactivityService) : base(logger)
        {
            _logger = logger;
            _lavaNode = lavaNode;
        ...
        public async Task LoopMusicCommandAsync(int amount = 1)
        {
            ...
            {
                await SendBasicErrorEmbedAsync("The player must be actively playing a track in " +
                                               "order to loop it.");
