...
namespace Kaguya.Discord
{
	public class KaguyaBase<T> : ModuleBase<ScopedCommandContext>
	{
		...
		{
			return await SendEmbedAsync(GetBasicErrorEmbedBuilder(description, mentionUser).Build());
		}
		...
		protected EmbedBuilder GetBasicErrorEmbedBuilder(string description, bool mentionUser = true)
		{
			return GetBasicEmbedBuilder("⛔ " + description, KaguyaColors.Red, mentionUser);
		}

		protected EmbedBuilder GetBasicSuccessEmbedBuilder(string description, bool mentionUser = true)
		{
			return GetBasicEmbedBuilder("👍 " + description, KaguyaColors.Green, mentionUser);
		}
		...
		protected async Task<RestUserMessage> SendAsync(string message)
		{
			...
			{
				_logger.LogError(e,
					$"Could not send message to channel [{Context.Channel} | {Context.Channel.Id}] " +
		...
		protected async Task<RestUserMessage> SetToInactiveEmbedAsync(RestUserMessage msg)
		{
			...
			{
				x.Content = null;
				x.Embed = new KaguyaEmbedBuilder(KaguyaColors.Orange).WithTitle("⏳ Expired!").Build();
			});
