...
using KaguyaProjectV2.KaguyaBot.Core.Commands;
...
namespace KaguyaProjectV2.KaguyaBot.Core.Handlers.WarnEvent
{
    ...
    {
        public WarnHandler(Server server, WarnedUser warnedUser, ICommandContext context)
        {
            var warn = new Warn();
            warn.OnWarning += async (server, warnedUser, context) =>
            {
