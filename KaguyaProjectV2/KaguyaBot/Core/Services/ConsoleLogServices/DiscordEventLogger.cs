...
namespace KaguyaProjectV2.KaguyaBot.Core.Services.ConsoleLogServices
{
    public class DiscordEventLogger
    {
        ...
        public static void InitLogger()
        {
            ...
            _client.ShardDisconnected += async (Exception ex, DiscordSocketClient client) =>
                await ConsoleLogger.LogAsync($"Shard {client.ShardId} disconnected. Exception: {ex.Message}", LogLvl.ERROR);

            _client.ShardReady += async (DiscordSocketClient client) =>
            ...
            _client.JoinedGuild += async (SocketGuild guild) =>
                await ConsoleLogger.LogAsync($"Joined Guild [Name: {guild.Name} | ID: {guild.Id}]", LogLvl.INFO);

            _client.LeftGuild += async (SocketGuild guild) =>
