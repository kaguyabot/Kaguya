        ...
        public async Task Command(params string[] args)
        {
            ...
            else
            {
                switch (args.Length)
                {
                    ...
                    {
                        List<Fish> allFishToSell = await DatabaseQueries.GetUnsoldFishForUserAsync(Context.User.Id);
                        ...
                        await InlineReactionReplyAsync(new ReactionCallbackData("", sellAllConfirmEmbed.Build(), true,
                                                       ...
                                                       {
                                                           foreach (Fish fish in allFishToSell)
...
#region If(mass-selling...)
            if (fishType != null)
            {
                ...
                List<Fish> unsoldFishToSell = await DatabaseQueries.GetUnsoldFishForUserAsync(user.UserId);
                ...
                await InlineReactionReplyAsync(new ReactionCallbackData("", massSellEmbed.Build(),
                                                   true, true, TimeSpan.FromSeconds(120))
                                               .AddCallBack(GlobalProperties.CheckMarkEmoji(), async (c, r) =>
                                               {
                                                   foreach (Fish fish in unsoldFishToSell)
                                                       DatabaseQueries.SellFish(fish, user.UserId);

                                                   user = await DatabaseQueries.GetOrCreateUserAsync(user.UserId);
                                                   int payout = Fish.GetPayoutForFish(unsoldFishToSell, user.FishExp);
